...
# fig2.savefig(os.path.join(this_dir,"..","images","DSFTmag3D_5pi.jpg"), dpi=2000)

# new_shape = (100,100)
# fig2, ax2 = plt.subplots()
# m = cm.ScalarMappable(cmap='coolwarm')
# m.set_array(np.abs(HHplot_negpi_pi))
# ax2.imshow(np.abs(HHplot_negpi_pi[~conds].reshape(new_shape)), cmap='coolwarm')
# cbar = fig2.colorbar(m)
# cbar.set_label(r"$|H(e^{j \mu}, e^{j \nu})|$")
# ax2.set_xticklabels(nuMesh[~conds].reshape(new_shape)[::10,1])
# plt.show()
